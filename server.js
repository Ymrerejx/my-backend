...
app.post('/addDay', (req, res) => {
  const { rating, readingChecked, sleepValue, Description, Fap, currentDate } = req.body;
  db.query('INSERT INTO Day (Rating, Reading, Sleep, Description, Fap, Date) VALUES (?, ?, ?, ?, ?, ?)', [rating, readingChecked, sleepValue, Description, Fap, currentDate], (err, result) => {
    if (err) {
      console.log(err);
...
app.post('/getToday', (req, res) => {
  const { currentDate } = req.body;
  db.query('SELECT Sport.Id, Day.Rating, Day.Sleep, Day.Reading, Day.Description, Day.Fap, Sport.Activity, Day.Date FROM Day LEFT JOIN Sport ON Day.Date = Sport.Date WHERE Day.Date = "' + currentDate+  '";', (err, result) => {
    if (err) {
      console.log(err);
...
app.post('/updateToday', (req, res) => {
    const {rating, readingChecked, sleepValue, Description, Fap, currentDate } = req.body;
  db.query('UPDATE Day SET Sleep = "'+sleepValue+'", Reading = '+readingChecked+', Rating = "'+rating+'", Description = "'+ Description+'", Fap = "'+Fap+'" WHERE Date = "'+currentDate+'";', (err, result) => {
    if (err) {
      console.log(err);
